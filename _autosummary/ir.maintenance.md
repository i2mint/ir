...
[`ir.policy`](ir.policy.md#module-ir.policy)). [`maintain()`](#ir.maintenance.maintain) reads each corpus’s resolved
[`MaintenancePolicy`](ir.policy.md#ir.policy.MaintenancePolicy) and does the work that is **due**:
...
- when `synopsis.enabled`, the rebuild’s strategy is wrapped in
  [`ir.with_synopsis()`](ir.md#ir.with_synopsis), so new / changed artifacts gain an LLM synopsis —
...
[`ir.schedule`](ir.schedule.md#module-ir.schedule) installs that entry for you (`ir schedule`) and reports on it;
