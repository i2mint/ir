...
|----------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------|
| [`IndexPlan`](#ir.base.IndexPlan)([filter_fields, surfaces])              | An [`IndexingStrategy`](ir.strategy.md#ir.strategy.IndexingStrategy)'s output for one artifact. |
...
[`ir.sources`](ir.sources.md#module-ir.sources)). [`ir.select`](ir.md#ir.select) re-exports this for disclosure.
...
[`ir.retrieve.fuse_hits()`](ir.retrieve.md#ir.retrieve.fuse_hits), which only compares scores within a source.
...
The single source of truth for ledger keying — [`ir.index.build()`](ir.index.md#ir.index.build)
writes entries under this key and readers (e.g.
[`ir.retrieve.records_for_artifact()`](ir.retrieve.md#ir.retrieve.records_for_artifact)) resolve them with it, so the
